"""
YouTube 채널 자막 자동 수집기 (원격/레포 버전)
- 경로를 GitHub 레포 기준으로 조정
- 원본: yt_collector.py (로컬 PKM 버전)

Usage:
    python3 yt_collector_remote.py              # 신규 영상만 수집
    python3 yt_collector_remote.py --all        # 인덱스 무시하고 전체 수집
"""
...
def main():
    ...
    log.info("=== YouTube 자막 수집 시작 (원격 버전) ===")
    ...
    for channel in channels:
        ...
        for video in videos:
            ...
            log.info(f"    저장: {filepath.relative_to(REPO_ROOT)}")

            collected.add(vid)
            new_count += 1
            new_files.append(str(filepath.relative_to(REPO_ROOT)))
